...
### Объявления
- `GET /api/ads` — список активных объявлений. Query: `limit` (1-100, по умолчанию 20), `offset`, `categoryId`, `subcategoryId`, `seasonCode`, `q` (поиск по названию/описанию), `minPrice`, `maxPrice`. Ответ: `{ items: Ad[] }`.
...
- `/market` — мастер просмотра свежих объявлений: выбор категории → подкатегории → вывод 5 новых объявлений с кнопкой «Написать продавцу» (deep-link tg://user?id=...).

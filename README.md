...
Основные страницы мини-приложения:

- `/` — лента объявлений, фильтры по категориям и цены
- `/favorites` — избранные объявления с возможностью снять/добавить сердечко
- `/orders` — заказы пользователя (подтягиваются по telegramId, есть демо-данные при ошибке)
- `/profile` — личный кабинет с контактными настройками и кнопкой перехода в чат бота
- `/seasons` / `/season/:code` — каркасы для сезонных витрин и их объявлений
...
- `GET /api/ads` — список активных объявлений
  - Query параметры: `limit`, `offset`, `categoryId`, `subcategoryId`, `seasonCode`, `sellerTelegramId`, `q`, `minPrice`, `maxPrice`
  - Ответ: `{ items: Ad[] }`
...
- `/market` — лента объявлений: выбор категории/подкатегории и свежие карточки с кнопкой «Написать продавцу»
...
Команда `/market` ведёт покупателя по трём шагам: выбор категории, затем подкатегории (или «Все подкатегории») и выдача 5–10 свежих объявлений с ценой, кратким описанием и кнопкой «Написать продавцу» (deep link `tg://user?id=<sellerTelegramId>`).
...
### Автоматизация веток
Добавлен helper для быстрого создания веток и пуша в origin:

```bash
./scripts/branch-helper.sh <type> <slug> [--base <branch>] [--push]
```

Подробнее — в `docs/git-branch-automation.md`.

...
const STORY_MAX_FILE_SIZE = 50 * 1024 * 1024;
const STORY_ALLOWED_MIME_TYPES = [
  'image/jpeg', 'image/png', 'image/webp', 'image/jpg',
  'video/mp4', 'video/quicktime', 'video/webm',
];

router.post('/story/presigned-url', telegramAuthMiddleware, async (req, res) => {
  const requestId = Date.now().toString(36);
  console.log(`[uploads][${requestId}] POST /story/presigned-url - start`);
  ...
  try {
    ...
    console.log(`[uploads][${requestId}] Story File: ext=${fileExtension}, mime=${mimeType}, size=${size}, type=${type}`);

    if (size && size > STORY_MAX_FILE_SIZE) {
      console.log(`[uploads][${requestId}] Rejected: file too large (${size} > ${STORY_MAX_FILE_SIZE})`);
      return res.status(400).json({ 
        error: 'FILE_TOO_LARGE',
    ...
    if (mimeType && !STORY_ALLOWED_MIME_TYPES.includes(mimeType.toLowerCase())) {
      console.log(`[uploads][${requestId}] Rejected: invalid mime type ${mimeType}`);
      return res.status(400).json({ 
        error: 'INVALID_FILE_TYPE',
        message: `Недопустимый тип файла. Разрешены: JPEG, PNG, WebP, MP4, MOV, WebM.`,
      });
    }
    ...
    console.log(`[uploads][${requestId}] Story Success - fileId: ${result.fileId}`);
    ...
    console.error(`[uploads][${requestId}] Story Error:`, error.message || error);
